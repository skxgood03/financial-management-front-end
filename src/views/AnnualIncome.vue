...
export default {
  ...
  setup(props: any) {
    ...
    const main = ref()
    const echartsfun = () => {
      const transactionPartners = props.data.annual_income.map((item: any) => item.month)
      const total_expense = props.data.annual_income.map((item: any) => item.daily_income)
      // const expense_count = props.data.sum_expense_by_category.map((item: any) => item.expense_count) // 添加支出次数数据
      const totalExpense = props.data.annual_income.reduce((sum: number, item: any) => sum + item.daily_income, 0);
